...
import { Bell, BarChart3, Settings, Download, Plus, X, Loader2 } from "lucide-react";
...
function Progress() {
  ...
  const handleAddSubtask = async () => {
    ...
    await addSubTask(newSubtask.trim(), activeTask.id);
    setNewSubtask("");
  };
        ...
        <div className="space-y-4">
          ...
          {activeTask ? (
            <div className="rounded-lg border border-zinc-800 bg-zinc-900 p-4">
              {/* Show loading state for subtasks */}
              {subTaskLoading ? (
                <div className="flex items-center justify-center py-8">
                  <Loader2 className="h-6 w-6 animate-spin text-purple-500" />
                  <span className="ml-2 text-sm text-zinc-400">Loading subtasks...</span>
                </div>
              ) : (
                <>
              ...
              {!subTaskLoading && (
                <div className="mt-4 flex gap-2">
                  ...
                  <Button 
                    ...
                    disabled={subTaskLoading || !newSubtask.trim()}
                  >
                    {subTaskLoading ? (
